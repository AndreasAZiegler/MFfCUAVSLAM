...
The culling during the map merging follows the same approach as the one used for the local \ac{KF} culling in ORB-SLAM \cite{Mur-Artal2015}: All the \acp{KF} are discarded which observe a certain percentage of \acp{MP}, the redundancy threshold, which have been seen in at least other three \acp{KF} in the same or finer scale.
