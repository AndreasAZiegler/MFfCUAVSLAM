...
\subsection{\acf{KF}}
\acfp{KF} are the most representative poses of the trajectory of a robot. As an example in the sketch in figure \autoref{fig:kf}, $x_0$ and $x_3$ are \acp{KF} whereas $x_1$ and $x_2$ are not \acp{KF} as they are may not enough representative.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{Two clients each with own landmarks and \acp{KF}}
  \label{fig:kfm1}
\end{figure}
...
\textcolor{red}{!!Check for illustration in [7] of ORB-SLAM.!!}

Co-visibility is represented as an undirected weighted graph. Each node is a \ac{KF} and an edge between two \acp{KF} exists if they share observations of the same \acp{MP}, being the weight of the edge the number of common \acp{MP} \cite{Mur-Artal2015}.
...
\begin{enumerate}
  \item {Two clients observed the same location, shown in figure \autoref{fig:kfm4}
    \begin{figure}[H]
      ...
      \label{fig:kfm4}
    \end{figure}}
  \item {The transformation between the two \acp{KF} can be obtained, shown in figure \autoref{fig:kfm5}
    \begin{figure}[H]
      \centering
...
To give the reader a better intuition about how the proposed map merging approach works, the new approach will be visualized in an example step by step.\\
The example starts with two \acp{KF}, one per client, which are checked if they observe the same location, shown in figure \autoref{fig:newapproach1}. If a \ac{KFM} was detected, as in \autoref{fig:newapproach2}, $m$ \acp{KF} are skipped before processing the next \acp{KF}, shown in \autoref{fig:newapproach3}.
...
\begin{figure}[H]
    ...
    \subcaptionbox{First \ac{KFM}\label{fig:newapproach2}}{
      ...
      \end{tikzpicture}}
      \caption{First two steps of the new map merging procedure}
...
After $m$ \acp{KF} are skipped, the system checks again, if a \ac{KFM} can be detected, shown in \autoref{fig:newapproach3}. \autoref{fig:newapproach4} shows the case, where the next \ac{KFM} could be detected.
...
Then again $m$ \ac{KF} are skipped, as shown in \autoref{fig:newapproach5}, before the last \ac{KFM} can be detected, shown in \autoref{fig:newapproach6}.
...
After the last \ac{KFM} was detected, the maps will be aligned with the transformation $T$ from the first \ac{KFM} and the \acp{MP} of the \acp{KF} belonging to a \ac{KFM} are fused, shown in \autoref{fig:newapproach7}.
...
After the two maps are aligned (possibly with still some miss alignment as in \autoref{fig:newapproach7}) the \ac{PGO} can be performed which optimizes the poses of the \acp{KF} and afterwards the \ac{BA} will be performed which optimizes the poses of both, of the \acp{KF} and of the \acp{MP}.\\
...
If \acp{KF} are skipped after \ac{KFM} was detected, the next \ac{KFM} will potentially have a bigger distance to the previous \ac{KFM}. This leads that the \acp{KFM} will be more apart from each other and therefor will cover a bigger area. The \acp{KFM} from a bigger area provide the \ac{PGO} and the \ac{BA} with more information which hopefully leads to a reduction in drift and an increased accuracy.
\textcolor{red}{\textbf{!!!Check \ac{PGO} implementation!!!}}

To visualize the influence of the skipping of \acp{KF}, \autoref{fig:kfskip1} shows the co-visibility graph of an experiment where only one \ac{KF} was skipped after a \ac{KFM} was detected. \autoref{fig:kfskip2} on the other hand shows an experiment where 10 \acp{KF} were skipped after a \ac{KFM} was detected. One can easily see, that the \acp{KFM} in \autoref{fig:kfskip2} are more apart from each other and cover a bigger area compared to the \acp{KFM} in \autoref{fig:kfskip1}.
