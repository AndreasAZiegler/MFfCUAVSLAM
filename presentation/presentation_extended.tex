...
\frame{
  \frametitle{Map merging - Results}

  \begin{table}[ht!]
  \begin{center}
  \begin{tabular}{c|c|c}
    \# \glspl{kfm} & \# \glspl{kf} skip & rmse\\ 
    \hline
    1 & 0 & 0.1311 \\
    5 & 20 & 0.0912 \\
    10 & 5 & 0.1236 \\
  ...
  \end{tabular}
  \end{center}
  \caption{Error (rmse) of different settings (\glspl{kfm} and \gls{kf} skipps).}
  \label{tab:results_new_approach}
  \end{table}
}
...
\frame{
  ...
  \begin{tabular}{c|c|c|c|c}
    Culling &  \# \glspl{kfm} & \# \glspl{kf} skip & \acrshort{pgo} \textcolor{orange}{v} / \textcolor{blue}{e} & \acrshort{ba} \textcolor{orange}{v} / \textcolor{blue}{e} \\ 
    \hline \hline
...
\frame{
  ...
  \visible<1->{
  \begin{block}{}
  Considerable computational benefits can be gained by substituting the \acrfull{lm} algorithm in the implementation of \acrfull{ba} with a variant of \acrfull{dl} non-linear least squares technique {\small\cite{Lourakis2005}}
  \end{block}
  }
  \only<2>{
...
\frame{
  \frametitle{Optimization - \gls{lm}}
  \visible<1->{
    The \gls{lm} solves iteratively $$(\mathbf{J}^T \mathbf{J} + \lambda \mathbf{I}) \delta = \mathbf{J}^T \boldsymbol{\epsilon} \text{, where } \boldsymbol{\epsilon} = [\mathbf{y} - \mathbf{f(\boldsymbol{\beta})}]$$
  }

  \only<2->{
  \begin{itemize}
    \visible<2->{ \item With a small $\lambda$ \gls{lm} becomes a Gauss-Newton method}
    \visible<3->{ \item With a big $\lambda$ \gls{lm} behaves like a Gradient-descent method}
    \visible<4->{ \item If an update doesn't reduce the error, $\lambda$ will be increased and the equation must be solved again}
  \end{itemize}
  }
...
\frame{
  \frametitle{Optimization - \gls{dl}}
  \visible<1->{
    The \gls{dl} solves iteratively
    $$\min_\delta 2(\frac{1}{2} \boldsymbol{\epsilon}^T \boldsymbol{\epsilon} - (\mathbf{J}\boldsymbol{\epsilon})^T \delta + \frac{1}{2} \delta^T \mathbf{J}^T \mathbf{J} \delta) \text{, subjected to } ||\delta|| \le \Delta$$

    For $\kappa \in [0, 2]$, the dog leg trajectory is defined as
    \begin{equation*}
      \mathbf{\delta}(\kappa) = 
      \begin{cases}
        \kappa \mathbf{\delta}_{gd} & 0 \le \kappa \le 1 \\
        \mathbf{\delta}_{gd} + (\kappa -1)(\mathbf{\delta}_{gn} - \mathbf{\delta}_{gd}) & 1 \le \kappa \le 2
      \end{cases}
    \end{equation*}
  }
}
...
\frame{
  ...
  the dog leg trajectory looks like\let\thefootnote\relax\footnote{Figure from \cite{Lourakis2005}}
  \begin{center}
    \includegraphics[scale=0.3]{gfx/dogleg}
  \end{center}
}
...
\frame{
  ...
  \only<1->{
  \begin{itemize}
    \visible<1->{ \item Once the Gauss-Newton step has been determined, the \gls{dl} algorithm can solve the subproblem for various $\Delta$ without resolving an equation}
    \visible<2->{ \item Reducing the number of times the Gauss-Newton step has to be determined is crucial for the overall performance of the minimization process}
    \visible<3->{ \item For the mentioned reasons the \gls{dl} algorithm requires less computational effort compared to the \gls{lm} algorithm}
  \end{itemize}
  }
}
