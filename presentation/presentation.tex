...
\frame{ \frametitle{Introduction}
	\begin{itemize}
	\visible<1->{
  \item A common map is required
  }
  \visible<2->{
  \item The maps need to be merged
  }
	\visible<3->{
	\item The alignment of them has to be found
	}
	\end{itemize}
}
\note[enumerate]
...
\begin{frame}
  ...
  \only<1->{
    \acrfull{kf}: The most ``representative'' poses\\ 
    \medskip
  }
  \only<2>{
  Two clients each with an own map and \acrfullpl{kf}
  \begin{center}
  \begin{tikzpicture}
    \coordinate (A) at (0, 0);
    \coordinate (B) at (3, 2);
  ...
  \only<4>{
    \acrfull{kfm}: A pair of two \acrfullpl{kf} observing the same location $\rightarrow$ Can obtain transformation
  }
...
\frame{ \frametitle{Motivation}
  ...
  \end{itemize}
  \visible<5>{\tiny{\cite{Mur-Artal2016}}}
}
...
\frame{
  ...
  \begin{tikzpicture}
    ...
    \begin{tabular}{cl}
      ...
      \tikz\node [fill=green!70, circle, inner sep=2pt, text width=0.1mm] {}; \tikz\node[fill=green!30, circle,inner sep=1pt, text width=0.1mm] {}; & Map Points (Land Marks)
    \end{tabular}
    };
...
\frame{
  ...
  \begin{block}{Co-visibility graph}
    Connections/Edges between \acrfullpl{kf} which observe the same map points (land marks)
  \end{block}
  }
  \only<2->{
...
\frame{
  ...
  \begin{center}
    ...
    \begin{tabular}{cl}
      \tikz\node[fill=orange!70, circle,inner sep=3pt, text width=0.1mm] {}; \tikz\node[fill=blue!70, circle,inner sep=3pt, text width=0.1mm] {}; & Key Frames of a Match \\
      \tikz\node[fill=orange!30, circle,inner sep=2.0pt, text width=0.1mm] {}; \tikz\node[fill=blue!30, circle,inner sep=2.0pt, text width=0.1mm] {}; & Key Frames \\
    \end{tabular}
    };
    \end{tikzpicture}
%    \begin{tikzpicture}[auto]
