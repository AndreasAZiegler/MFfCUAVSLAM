...
//      mpComm = rhs.mpComm;
      mspComm = rhs.mspComm;
      mId = move(rhs.mId);
      mTimeStamp = move(rhs.mTimeStamp);
      mUniqueId = move(rhs.mUniqueId);
      mVisId=move(rhs.mVisId);
      mnGridCols = move(rhs.mnGridCols);
      mnGridRows = move(rhs.mnGridRows);
      mfGridElementWidthInv = move(rhs.mfGridElementWidthInv);
      mfGridElementHeightInv = move(rhs.mfGridElementHeightInv);
      fx = move(rhs.fx);
      fy = move(rhs.fy);
      cx = move(rhs.cx);
      cy = move(rhs.cy);
      invfx = move(rhs.invfx);
      invfy = move(rhs.invfy);
      N = move(rhs.N);
      mvKeys = move(rhs.mvKeys);
      mvKeysUn = move(rhs.mvKeysUn);
      mDescriptors = move(rhs.mDescriptors);
      mK = move(rhs.mK);
      mnScaleLevels = move(rhs.mnScaleLevels);
      mfScaleFactor = move(rhs.mfScaleFactor);
      mfLogScaleFactor = move(rhs.mfLogScaleFactor);
      mvScaleFactors = move(rhs.mvScaleFactors);
      mvLevelSigma2 = move(rhs.mvLevelSigma2);
      mvInvLevelSigma2 = move(rhs.mvInvLevelSigma2);
      mnMinX = move(rhs.mnMinX);
      mnMinY = move(rhs.mnMinY);
      mnMaxX = move(rhs.mnMaxX);
      mnMaxY = move(rhs.mnMaxY);
      mbSentOnce = move(rhs.mbSentOnce);
      mbIsEmpty = move(rhs.mbIsEmpty);
      mBowVec = move(rhs.mBowVec);
      mFeatVec = move(rhs.mFeatVec);
      mTcp = move(rhs.mTcp);
      Tcw = move(rhs.Tcw);
      Twc = move(rhs.Twc);
      Ow = move(rhs.Ow);
      ...
      mConnectedKeyFrameWeights = rhs.mConnectedKeyFrameWeights;
      mvpOrderedConnectedKeyFrames = rhs.mvpOrderedConnectedKeyFrames;
      mvOrderedWeights = move(rhs.mvOrderedWeights);
      mbFirstConnection = rhs.mbFirstConnection;
      mpParent = rhs.mpParent;
      mspChildrens = rhs.mspChildrens;
      mspLoopEdges = rhs.mspLoopEdges;
