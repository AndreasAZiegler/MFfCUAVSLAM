...
MapMerger::mapptr MapMerger::MergeMaps(mapptr pMapCurr, mapptr pMapMatch, vector<MapMatchHit> vMatchHits)
{
    ...
    for(set<ccptr>::iterator sit = spCCC.begin();sit!=spCCC.end();++sit)
    {
        ...
        switch(pCC->mClientId)
        {
            case(static_cast<size_t>(0)):
                ...
                b0 = true;
                while(!pCC->LockComm()){usleep(pCC->mLockSleep);}
                while(!pCC->LockMapping()){usleep(pCC->mLockSleep);}
                while(!pCC->LockPlaceRec()){usleep(pCC->mLockSleep);}
                break;
    ...
    while(!pMapCurr->LockMapUpdate()){usleep(pMapCurr->GetLockSleep());}
    while(!pMapMatch->LockMapUpdate()){usleep(pMapMatch->GetLockSleep());}
    cout << "locked" << endl;
    ...
    while(!pFusedMap->LockMapUpdate()){usleep(pFusedMap->GetLockSleep());}
    ...
    std::vector<g2o::Sim3> g2oS_wm_wc_vec;
...
//    for(int idx = 0;idx < 1;++idx)
      cout << ">>>>> MapMerger::MergeMaps - process hit #" << count1 << endl;
...

			{
				...
				g2oS_wm_wc_vec.push_back(g2oS_wm_wc);
			}
