...
bool MapMatcher::DetectMatches()
{
    ...
    // If there are no match candidates, just add new keyframe and return false
...
void MapMatcher::CorrectLoop()
{
    ...
    mFoundMatches[pMatchedMap][mpCurrMap].push_back(MMH);
		...
		if(mFoundMatches[mpCurrMap][pMatchedMap].size() >= mMMParams.mMinHitsForMerge) {
			...
			//mpMapMerger->globalBundleAdjustment(mpCurrMap, pMatchedMap, pMergedMap, vMatches);
    }
