...
//            if(mpComm->mbStrictLock) unique_lock<mutex> lockComm(mpComm->mMutexForMapping);
            if(mVerboseMode == -9) cout << "xxx Mapping --> Lock Mapping xxx" << endl;
            if(mVerboseMode == -9) cout << "LockSleep: " << mpCC->mLockSleep << endl;
            if(mpComm->mbStrictLock) while(!mpCC->LockMapping()){usleep(mpCC->mLockSleep);}
            if(mVerboseMode == -9) cout << "xxx Mapping --> Mapping Locked xxx" << endl;
...
//                KeyFrameCulling();
            ...
            if(mpComm->mbStrictLock) mpCC->UnLockMapping();
        }
        else if(Stop())
        {
            // Safe area to stop
...
//            unique_lock<mutex> lockMapUpdate(mpMap->mMutexMapUpdate);
            ...
            if(mVerboseMode == -9) cout << "xxx Mapping --> MapUpdate Locked xxx" << endl;
...
//    mbAbortBA=true;
    ...
    if(pKF->IsEmpty()) cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m Client " << mClientId << ": In \"LocalMapping::ProcessNewKeyFrame()\": empty KF sent to mapping" << endl;
    if(pKF->isBad()) cout << "\033[1;33m!!! WARN !!!\033[0m Client " << mClientId << ": In \"LocalMapping::ProcessNewKeyFrame()\": bad KF sent to mapping" << endl;
}
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    {
        ++mAddedKfs;
        if(mpCurrentKeyFrame->isBad()) cout << "\033[1;33m!!! WARN !!!\033[0m Client " << mClientId << ": In \"LocalMapping::ProcessNewKeyFrame()\": bad KF processed by mapping" << endl;
    }
...
//            ++mCulledKfs;
//            if(mVerboseMode > 0) cout << "Mapping Client " << mClientId <<": Added|Culled KFs: " << mAddedKfs << "|" << mCulledKfs << "--> " << static_cast<double>(mCulledKfs)/static_cast<double>(mAddedKfs) << "%" << endl;
...
//            if(mVerboseMode > 0) cout << "Mapping Client " << mClientId <<": culled KF stats: Mean|StdDev: " << mCulledKfsRatioMean << "|" << mCulledKfRatioStdDev << endl;
        }
    }
}
...
void LocalMapping::ResetIfRequested()
{
    ...
    if(mbResetRequested)
    {
        if(mVerboseMode > 0) cout << "ClientHandler " << mClientId << ": LocalMapping --> Reset" << endl;
        mlNewKeyFrames.clear();
        mlpRecentAddedMapPoints.clear();
        #ifndef MAPFUSION
        if(mpLoopFinder)
        {
            if(mVerboseMode > 0)  cout << "ClientHandler " << mpCC->mClientId << ": LocalMapping::ResetIfRequested --> Request Loop Closure module reset" << endl;
            mpLoopFinder->RequestReset();
            if(mVerboseMode > 0)  cout << "ClientHandler " << mpCC->mClientId << ": LocalMapping::ResetIfRequested --> finished Loop Closure module reset" << endl;
        }
        #endif
        if(mVerboseMode > 0) cout << "ClientHandler " << mClientId << ": LocalMapping --> Resetting finished" << endl;
