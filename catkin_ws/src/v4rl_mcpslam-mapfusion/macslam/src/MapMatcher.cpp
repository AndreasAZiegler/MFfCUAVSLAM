...
void MapMatcher::CorrectLoop()
{
    ...
    if(mMMParams.mMinHitsForMerge > 1) cout << "\033[1;33m!!! WARN !!!\033[0m In \"MapMatcher::CorrectLoop()\": map matching / merging algorithm not properly implemented for usin >1 place recognition hit" << endl;
    ...
    if(mFoundMatches[mpCurrMap][pMatchedMap].size() >= mMMParams.mMinHitsForMerge)
    {
        ...
        mapptr pMergedMap = mpMapMerger->MergeMaps(mpCurrMap, pMatchedMap, vMatches.front(), g2oScw);
...
void MapMatcher::PublishMergedMap(mapptr pMap, set<size_t> suAssClientsC, set<size_t> suAssClientsM)
{
    ...
    CovMsgCM.header.frame_id = "odomX";
    ...
    for(vector<kfptr>::iterator vit=vpKFs.begin();vit!=vpKFs.end();++vit)
    {
        if(((*vit)->IsEmpty()) || ((*vit)->isBad())) continue;
        ...
        for(set<kfptr>::iterator sit=vConKFs.begin();sit!=vConKFs.end();++sit)
        {
            ...
            if(CovMat[CurIdKf][CurIdCon]==true || CovMat[CurIdCon][CurIdKf]==true) continue;
            ...
            if(!(suAssClientsC.count(pKF->mId.second)) && !(suAssClientsM.count(pKF->mId.second))) cout << "\033[1;31m!!! ERROR !!!\033[0m In \"MapMatcher::PublishMergedMap(...)\": pKF: ClientId neither IDC nor IDM" << endl;
            if(!(suAssClientsC.count(pCon->mId.second)) && !(suAssClientsM.count(pCon->mId.second))) cout << "\033[1;31m!!! ERROR !!!\033[0m In \"MapMatcher::PublishMergedMap(...)\": pCon: ClientId neither IDC nor IDM" << endl;
